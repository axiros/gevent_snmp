...
#### as_list
If given and set to ```True``` the result is return as a list of ```(oid,
value)``` tuples.

#### get_netsnmp_string
If given and set to ```True``` each result value is converted to a string.
...
Control the format of the values if the flag ```get_netsnmp_string=True``` is
...
Furthermore these options are global for the entire process.
So they affect all AsyncSession() at the same time.
Hence the recommendation is to set them once at process startup.
